...
注意：判断时需结合该公司所属行业特征，避免一刀切；项目制、重资产、高周转行业应有不同的风险阈值）：
1. 【收入质量】应收账款同比增速是否显著超过营收同比增速？（若是，可能存在以放宽信用换营收的粉饰行为）
2. 【运营效率】存货同比增速是否远超营收同比增速？（若是，提示去库存压力或销售放缓；需区分主动备货与被动积压）
3. 【利润含金量】经营现金流/净利润是否显著低于 1.0？（若是，提示利润缺乏现金支撑；注意：项目制行业可能天然偏低）
4. 【回款质量】销售收现/营收是否显著低于 1.0？（若是，提示回款质量差，收入确认可能存在水分）
5. 【费用控制】三费率合计是否同比大幅上升？（关注费用增速是否超过营收增速，而非绝对值高低）
6. 【杠杆风险】资产负债率是否处于高位？（若超过 70% 需警惕，重资产行业可适当放宽）
7. 【利润真实性】净利润增速与营收增速是否出现显著背离？（净利增速远超营收增速需关注非经常性损益或毛利率异常）

最后列出 3 个最值得警惕的科目，并基于具体数字解释为何值得关注。`;
}
